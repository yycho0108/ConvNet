...
void activate(Matrix& I, Matrix& O, double f(double)){
	auto s = I.size();
	auto n_elem = s.n * s.m;
	activate<<<1,n_elem>>>(I.d_data(),O.d_data(),f);
...
void ActivationLayer::setup(Size& _s, int& _d){
	...
	for(int i=0;i<d;++i){
		...
		G.push_back(Matrix::empty(s));
		O.push_back(Matrix::empty(s));
	}

}

void ActivationLayer::outDim(Size& _s, int& _d){
	_s = s;
	_d = d; //unchanged
}
